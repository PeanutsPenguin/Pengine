...
namespace Pengine::Resources
{
	...
	inline std::shared_ptr<_ResourceType> PenResourcesManager::createResourceFromFile(const char* sourcePath, const char* destinationPath, Args ...data)
	{
		...
		std::filesystem::path destination = destinationPath + fullname;	//*/xxx.penfile
		...
		std::string_view destinationString = destination.string();

		//Check if ressources doesn't exist
		auto it = m_pathfileToId.find(destinationString);
		...
		if (!ptr->createResource(destination.string().c_str(), copyEmplacement.string().c_str(), data...))
		{
			m_currentId--;
			return nullptr;
		}

		m_idToPathfile[m_currentId] = destinationString;
		m_resourceStocker[m_currentId] = ptr;
		m_pathfileToId[destinationString] = m_currentId;
